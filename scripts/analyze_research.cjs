...
const keywords = [
    ...
    // Study Techniques
    'skim', 'scan', 'review', 'summarize', 'note-taking', 'underlining', 'cramming', 'pacing', 'rhythm',
...
try {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.trim().length > 20) { // skip very short noise
            ...
            if (hasKeyword) {
                // Add context (prev 2 lines, current, next 2 lines) to get full thoughts
                if (i > 1) linesToInclude.add(i - 2);
                if (i > 0) linesToInclude.add(i - 1);
                linesToInclude.add(i);
                if (i < lines.length - 1) linesToInclude.add(i + 1);
                if (i < lines.length - 2) linesToInclude.add(i + 2);
            }
        }
    }
    ...
    sortedIndices.forEach(index => {
        ...
        // Only add if it's not just a newline
