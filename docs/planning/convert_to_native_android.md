...
2. **Tier 2 — our mirror**: the R2 bucket holding the full quality-gated catalog (~56k books, ~39GB, built by `scripts/mirror-all.mjs`). The web app reads it via `VITE_BOOK_BASE`; the native app reads the same bucket via its own config constant. Bundle the hot set (curated front table + user's saved/in-progress, roughly 15 to 30MB) into the APK or first-run download so the home experience is instant offline.
