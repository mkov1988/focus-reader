...
- **The responsive book lift, on device.** The web mobile pickup was tabled with known rough edges ("it mostly works, still a few issues, but i think we just table for now as it'll prbably change when we flip to android"), and the native pickup does not exist yet. When it is built, FOLLOW_TAU 0.17 weight is the protected reference feel.
...
- **Possible stutter at 800+ wpm** from the once per word JS hop in the native architecture; the known fix is fully native word rendering, queued and unverified.
...
- **If random stops ever return on native**, the named next suspect is the Today screen quietly re rendering behind the reader every ~2s from the periodic store write; and a crash safe form of control bar redraw trimming was promised if words still hitch.
...
- **Book pickup and the open transition** exist only on web today. The native rebuild is deferred by design (never port the 544 line BookOpenTransition as is; rebuild simpler, native, with the tuned constants as the starting recipe).
